...
- 🗂 **卡片式管理**：项目名称、简介、路径（点击复制）、端口状态（逐端口探测）、依赖项、标签、备注，一卡尽览；🎨 卡片可自选七种背景色，运行中的卡片亮色描边并置顶
- 📚 **分组**：自建分组，项目拖进去、组内拖动卡片调整顺序、分组可重命名 / 拖动排序 / 一键解散（项目自动移回未分组）
...
- 🐳 **三种启动器**：
  ...
  - `static` —— 纯静态页面，一键打开
- ⌨️ **命令行 Agent 接入**：dsh、pi 这类 TUI 编码代理也能成为卡片——「启动」在新终端窗口拉起（等同手敲命令）；在终端手动启动的实例同样能被识别状态、一键停止
- 🤖 **AI 同事接入**：REST API + 自描述 Schema，其他 AI 建完新项目后，一个 HTTP 请求即可注册进工作台
- 🔁 **自举管理**：工作台把自己也注册成一张卡片——「停止」会优雅退出，「重启」通过引导进程自动拉起新实例（页面短暂断开后恢复）
- 🧩 **可扩展**：新增启动器类型 = 一个文件 + 一行注册，前端按钮按能力声明自动渲染
- 📋 **日志查看**：每个项目的启动输出就地查看（内存环形缓冲 + 落盘 `data/logs/`）
- 🌗 **亮色 / 深色主题**：顶栏一键切换，默认跟随系统偏好，手动选择会记住
...
| `options.processMatch` | 按命令行关键字匹配进程（停止兜底，如 `daemon.mjs`）；**无端口项目还用它判定运行状态**——即使在终端手动启动的实例也能被识别（命令行 Agent 类工具必备） | — | — |
